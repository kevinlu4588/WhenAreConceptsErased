...
def create_subset(target_label, root_dir, out_root, n_neg=5000):
    ...
    for shard_name in tqdm(sorted(shard_dirs), desc="Scanning for positives"):
        ...
        try:
            ...
            print(f"   ⚠️ Skipping {shard_name}: {e}")
            continue
    ...
    if not pos_collected:
        print(f"⚠️ No positives found for '{target_label}' (id={target_id}). Exiting.")
        sys.exit(1)
    ...
    if not neg_candidates:
        print(f"❌ No negative samples found. Exiting.")
        return
    ...
    if len(neg_ds_all) > n_neg:
        ...
        neg_ds = neg_ds_all
        print(f"   ⚠️ Only found {len(neg_ds)} negatives (requested {n_neg})")
